...
def match_sample_against_database(db_path, sample_file):
    """
    Match a sample file against all songs in the database.
    
    Args:
        db_path (str): Path to the SQLite database.
        sample_file (str): Path to the sample audio file to match.
    
    Returns:
        list: List of tuples (song_title, artist_name, match_percentage) sorted by match percentage.
    """
    # Calculate fingerprint for the sample
    sample_fingerprint = calculate_fingerprint(sample_file)
    if not sample_fingerprint:
        print(f"Failed to calculate fingerprint for {sample_file}")
        return []
    ...
    try:
        ...
        # Get all fingerprints in the database along with song info
        cursor.execute('''
            SELECT s.song_title, s.artist_name, f.fingerprint, s.song_id
            FROM fingerprints f
            JOIN samples smp ON f.sample_id = smp.sample_id
            JOIN songs s ON smp.song_id = s.song_id
        ''')
        
        results = cursor.fetchall()
        
        for song_title, artist_name, db_fingerprint, song_id in results:
            # Use sliding window match for better accuracy
            match_percentage, _ = sliding_window_match(db_fingerprint, sample_fingerprint)
            matches.append((song_title, artist_name, match_percentage, song_id))
        
        # Sort by match percentage in descending order
        matches.sort(key=lambda x: x[2], reverse=True)
        
        # Print matches above a certain threshold
        print(f"\nMatches for {os.path.basename(sample_file)}:")
        print("=" * 60)
        print(f"{'Song Title':<30} {'Artist':<20} {'Match %':<10}")
        print("-" * 60)
        
        for song_title, artist_name, match_percentage, _ in matches[:10]:  # Show top 10
            if match_percentage > 1.0:  # Only show reasonable matches
                print(f"{song_title[:30]:<30} {artist_name[:20]:<20} {match_percentage:.2f}%")
        ...
        print(f"Database error while matching sample: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def sliding_window_match(full_fingerprint, sample_fingerprint, window_step=10):
    """
    Uses a sliding window approach to find the best match between a sample fingerprint
    and a full song fingerprint.
    
    Args:
        full_fingerprint (str): The fingerprint of the full song.
        sample_fingerprint (str): The fingerprint of the sample.
        window_step (int): The step size for sliding the window.
        
    Returns:
        tuple: (best_match_percentage, best_position) where best_match_percentage is the 
               highest percentage match found and best_position is the position in the 
               full fingerprint where the best match starts.
    """
    if not full_fingerprint or not sample_fingerprint:
        return 0.0, 0
    
    sample_length = len(sample_fingerprint)
    full_length = len(full_fingerprint)
    
    # If sample is longer than the full fingerprint, truncate it
    if sample_length > full_length:
        sample_fingerprint = sample_fingerprint[:full_length]
        sample_length = full_length
    
    best_match = 0.0
    best_position = 0
    
    # Slide the window through the full fingerprint
    for pos in range(0, full_length - sample_length + 1, window_step):
        window = full_fingerprint[pos:pos + sample_length]
        match_percentage = calculate_percentage_match(window, sample_fingerprint)
        
        if match_percentage > best_match:
            best_match = match_percentage
            best_position = pos
    
    # Fine-tune around the best position found
    start_pos = max(0, best_position - window_step)
    end_pos = min(full_length - sample_length + 1, best_position + window_step + 1)
...
def main(db_path, input_folder, output_folder, match_mode=False, sample_to_match=None):
    """
    Main function to either process audio files or match a sample against the database.

    Args:
        db_path (str): Path to the SQLite database.
        input_folder (str): Path to the folder containing audio files.
        output_folder (str): Path to the folder to save the samples.
        match_mode (bool): If True, match the sample against the database.
        sample_to_match (str): Path to the sample file to match (only used if match_mode is True).
    """
    ...
    if match_mode and sample_to_match:
        # Match single sample against the database
        print(f"Matching sample {sample_to_match} against database...")
        match_sample_against_database(db_path, sample_to_match)
    else:
...
if __name__ == "__main__":
    ...
    db_file = "./tunetrack.db"  
    input_folder = "./samples"  
    output_folder = "./samples/wav"  
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--match":
        if len(sys.argv) > 2:
            ...
            if os.path.isfile(sample_file):
                # Run in match mode
                main(db_file, input_folder, output_folder, match_mode=True, sample_to_match=sample_file)
            else:
                print(f"Error: Sample file {sample_file} does not exist.")
        else:
            print("Error: Please provide a sample file path.")
            print("Usage: python working3.py --match <sample_file_path>")
    else:
        # Run in normal processing mode
